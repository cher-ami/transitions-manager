...
const waiting = (time: number): Promise<void> =>
  new Promise((resolve) => setTimeout(resolve, time));

const mockUsePlayIn = vi.fn();
const mockUsePlayInComplete = vi.fn();
const mockUsePlayOut = vi.fn();
const mockUsePlayOutComplete = vi.fn();
...
beforeEach(() => {
  ...
  App = ({ transitionDuration = 0 }) => {
    ...
    return (
      <div className="App">
        <button onClick={clickPlayIn} data-testid={"play-in"}>
          play-in
        </button>
        <button onClick={clickPlayOut} data-testid={"play-out"}>
          play-out
        </button>
  ...
  Header = TransitionsHoc(
    (props: {
      ...
      return (
        <header className={props.className} data-testid={"header"}>
          Header
        </header>
      );
    },
