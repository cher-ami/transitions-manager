 ...
 */
export function Beeper<S = any, O extends Record<string, any> = {}>(
  initialState: S = null,
  initialOptions?: O
): {
  off,
  ...
  const on = (
    handler: THandler<S, O>
  ): ((handler?: THandler<S, O>) => void) => {
  ...
  const dispatch = (state?: S, options?: O): THandlers<S, O> => {
    ...
    return handlers.map((handler: THandler<S, O>) =>
      handler(state, currentOptions)
    );
  };
